...
<li class="act_item" id="act_item_<?php echo $a->id;?>">
    <div class="middle_portrait">
        <div class="portrait_wrap">
            <img class="border" src="<?php echo $a->organizer->profile->portrait; ?>" />
        </div>
...
?>
        ...
        <div class="location">
            地点：<?php echo $a->province === '待定' ? '待定' : "{$a->province}&nbsp;{$a->city}"; ?>
        </div>
        <div class="tool_bar">
<?php
echo '<span><a id="comm_'.$a->id.'_0" class="comment" href="javascript:void(0);">评论</a></span>';
?>
<?php
if($a->organizer->id != Yii::app()->session['user']->id){
    ...
    if($joined){
        echo '<span><a id="quit_'.$a->id.'" class="quit" href="javascript:void(0);">退出</a><i>|</i></span>';
    } else {
        echo '<span><a id="join_'.$a->id.'" class="join" href="javascript:void(0);">申请加入</a><i>|</i></span>';
    }
} else {
    echo '<span><a id="del_'.$a->id.'" class="del" href="javascript:void(0);">删除</a><i>|</i></span>';
    echo '<span><a id="edit_'.$a->id.'" class="edit" href="javascript:void(0);">编辑</a><i>|</i></span>';
}
?>
        </div>
...
?>
        <div class='needappr'>
            <span class='needappr_word'>待审批列表<!--<img class='needappr_arrow' src='#'>--></span>
            <ul>
