...
class SimpleHTMLDOMNode
{
    ...
    function innertext()
    {
        ...
        if (isset($this->_[HDOM_INFO_TEXT])) return $this->dom->restore_noise($this->convert_text($this->_[HDOM_INFO_TEXT]));
...
//        $pattern = "/([\w-:\*]*)(?:\#([\w-]+)|\.([\w-]+))?(?:\[@?(!?[\w-]+)(?:([!*^$]?=)[\"']?(.*?)[\"']?)?\])?([\/, ]+)/is";
    ...
    function convert_text($text)
    {
        ...
        if($sourceCharset == 'GB2312' && $targetCharset == 'UTF-8'){
            $targetCharset .= '//IGNORE';
        }
        ...
        if (!empty($sourceCharset) && !empty($targetCharset) && (strcasecmp($sourceCharset, $targetCharset) != 0))
        {
            ...
            if ((strcasecmp($targetCharset, 'UTF-8') == 0) && (strcasecmp($sourceCharset, 'UTF-8') == 0))
            {
                $converted_text = $text;
            }
            else
